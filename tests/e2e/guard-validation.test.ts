...
describe('Guard Validation', () => {
  ...
  describe('has_pr guard', () => {
    it('should pass when task has a PR link', async () => {
      ...
      const result2 = await ctx.pipelineEngine.executeTransition(result1.task!, 'in_review');
  ...
  describe('dependencies_resolved guard', () => {
    it('should pass when all dependencies are in final status', async () => {
      ...
      const r1 = await ctx.pipelineEngine.executeTransition(dep, 'in_progress');
      await ctx.pipelineEngine.executeTransition(r1.task!, 'done');

      // Now transition the dependent task
      const result = await ctx.pipelineEngine.executeTransition(task, 'ready');
      expect(result.success).toBe(true);
    });

    it('should fail when dependencies are not resolved', async () => {
      const customPipeline = await ctx.pipelineStore.createPipeline({
  ...
  describe('custom guard registration', () => {
    it('should allow registering and using custom guards', async () => {
      ...
      const task1 = await ctx.taskStore.createTask(createTaskInput(projectId, customPipeline.id));
      const result1 = await ctx.pipelineEngine.executeTransition(task1, 'assigned');
      ...
      const result2 = await ctx.pipelineEngine.executeTransition(task2, 'assigned');
      expect(result2.success).toBe(true);
    });
  });
});
