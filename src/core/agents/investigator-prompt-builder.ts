...
export class InvestigatorPromptBuilder extends BaseAgentPromptBuilder {
          ...
          proposedOptions: {
            ...
            items: {
              ...
              properties: {
                ...
                size: { type: 'string', enum: ['S', 'M', 'L', 'XL'], description: 'Size tier for this option: S (small/quick fix), M (medium/balanced), L (large/architectural), XL (extra-large/multi-phase). Determines the default routing target after selection.' },
              },
  ...
  private getReportStructureInstructions(): string[] {
    return [
      ...
      `- **size**: One of "S", "M", "L", or "XL" — determines the default routing target (S→implementing, M→planning, L→designing, XL→designing)`,
      ``,
      `Typical options:`,
