...
export class PostRunExtractor {
  ...
  async extractPlan(
    ...
    if (reportContent) {
      ...
      // Write subtasks/phases to task store (non-doc fields)
      ...
      // Primary write to task_docs table
      ...
      // Write fallback content to task_docs table
      if (fallbackContent) {
        await this.upsertTaskDoc(
          taskId,
          isInvestigator ? 'investigation_report' : 'plan',
  ...
  /**
   * Extract UX design metadata and spec from a successful ux-designer run.
   * Persists two doc types: `ux_design` (options metadata) and `ux_design_spec` (design spec for implementor).
   */
  async extractUxDesign(
    taskId: string,
    result: AgentRunResult,
    agentType: string,
    onLog: OnLog,
    ...
    if (result.exitCode !== 0 || agentType !== 'ux-designer') {
      onPostLog?.('extractUxDesign skipped (not applicable)', { agentType, exitCode: result.exitCode });
      return;
    }
    const _start = performance.now();
    ...
    if (so?.designOverview || so?.options) {
      ...
      await this.upsertTaskDoc(taskId, 'ux_design', uxDesignContent, summary, onLog);
    }
    ...
    if (so?.designSpec) {
      ...
      await this.upsertTaskDoc(taskId, 'ux_design_spec', so.designSpec, specSummary, onLog);
    }

    // Mark ux_design_feedback as addressed after successful run
    if (agentRunId) {
      try {
        await this.markFeedbackAsAddressed(taskId, ['ux_design_feedback'], agentRunId, onLog);
      } catch (err) {
  ...
  private async upsertTaskDoc(
    ...
    try {
      ...
      // Non-fatal — don't block pipeline on task_docs failure
