...
export function registerChatSessionHandlers(api: ApiClient): void {
  ...
  registerIpcHandler(IPC_CHANNELS.CHAT_SESSION_CREATE, async (_, input: { scopeType: string; scopeId: string; name: string; agentLib?: string; threadIntent?: string }) => {
  ...
  registerIpcHandler(IPC_CHANNELS.CHAT_SESSION_UPDATE, async (_, sessionId: string, input: { name?: string; agentLib?: string | null; permissionMode?: PermissionMode | null; systemPromptAppend?: string | null; draft?: string | null; threadIntent?: string | null }) => {
