...
function getAddInvestigateModeSql(): string {
  ...
  const investigateMode = {
    ...
    promptTemplate: [
      ...
      '   - `npx agents-manager tasks get {taskId} --json` — full task details',
      '   - `npx agents-manager events list --task {taskId} --json` — task event log',
...
function getUpdateAgentPromptTemplatesSql(): string {
  const ts = Date.now();

  // --- agent-def-claude-code: plan, plan_revision, investigate, technical_design, technical_design_revision + resume modes ---
  const claudeCodeModes = JSON.stringify([
    {
      mode: 'plan',
      promptTemplate: [
        'You are a senior software engineer. Analyze this task and create a detailed implementation plan. Task: {taskTitle}.{taskDescription}',
        '',
        '## Instructions',
  ...
  // --- agent-def-implementor: implement, request_changes + resume modes ---
  const implementorModes = JSON.stringify([
    {
      mode: 'implement',
      promptTemplate: [
    ...
    {
      mode: 'implement_resume',
      promptTemplate: [
        'Continue implementing the changes for this task using the user\'s decisions. Task: {taskTitle}.{taskDescription}',
        '{subtasksSection}',
        '{planSection}',
        '{technicalDesignSection}',
        '',
