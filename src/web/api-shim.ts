...
export function createWebApiShim(daemonUrl: string, daemonWsUrl: string): ApiShape {
  ...
  return {
    ...
    chat: {
      ...
      sessionStatus: (sessionId) => api.chat.getSessionStatus(sessionId),
    },

    // ── Chat Sessions ─────────────────────────────────────────────────
    chatSession: {
      create: (scopeType, scopeId, name, agentLib?, threadIntent?) =>
        api.chat.createSession({ scopeType, scopeId, name, agentLib, threadIntent }) as Promise<never>,
